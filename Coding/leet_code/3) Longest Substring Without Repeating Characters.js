...
//------------------------------------------------------------------------------------------------//
// // Function to assign a value of one variable to another variable 
// function copy(x) {
...
let subStr = new Array() , reqStr = new Array(); // initiating 2 arrays
...
for (let i = 0; i < s.length; i++) {  // for each character of the given string
    ...
    if (reqStr.length < subStr.length) { // if length of reqStr Array(required string) is smaller than subStr Array(sub string)
        // reqStr = copy(subStr) // Another way to copy.... any value from one variable to another variable [function written above]
    reqStr = subStr.slice(0) // then copy the values from subStr Array to reqStr Array
}
}
